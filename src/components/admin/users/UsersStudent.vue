  ...
  <section class="bg-main h-full">
    ...
    <div class="p-7">
      <div class="flex justify-between">
        <div>
          <p class="text-2xl font-medium">Student List</p>
      ...
      <div v-if="form">
        ...
        <form @submit.prevent="handleSubmit" class="my-10 w-3/5 mx-auto">
          ...
          <label>Role:</label>
          <input v-model="user.role" type="text" required class="w-full border py-2 px-3 mt-2 mb-5 border-gray-300 rounded-md">
        ...
        <div class="flex flex-wrap mt-10">
          <div v-for="student in students" :key="student.id" class="bg-white rounded-md border shadow mr-5 mb-5">
            ...
            <div class="flex justify-between items-center border-t ">
              ...
              <div class="w-1/3 py-5 px-7 cursor-pointer hover:bg-gray-100 hover:text-main-text"  @click="deleteUser(student.user.id)">
...
export default {
  ...
  methods: {
    ...
    getStudents() {
      fetch('http://91.201.214.131:8080/students')
        .then(response => response.json())
        .then(data => {
          this.students = data; // сохраняем полученные данные в свойство компонента
        });
    },
    deleteUser(id) {
  ...
  mounted() {
    this.getStudents(); // вызываем метод для получения данных при монтировании компонента
  },
  components: {
    NavBar
  }
        ...
        <div class="bg-white rounded-md my-10 pb-5">
          ...
          <form @submit.prevent="createAdmin" method="post" class="my-10 px-5 w-1/2 mx-auto flex flex-wrap">
            ...
            <label for="email" class="font-semibold">Email
              <input v-model="user.email" type="email" required placeholder="Email" class="w-full border py-2 px-3 mt-2 mb-5 border-gray-300 rounded-md" />
            </label>
            <label for="password" class="font-semibold mr-3">Password
              <input v-model="user.password" type="password" required placeholder="Enter Password" class="w-full border py-2 px-3 mt-2 mb-5 border-gray-300 rounded-md"   />
            </label>
            <label for="password" class="font-semibold mr-3">Role
              <input v-model="user.role" type="number" required placeholder="Enter Password" class="w-full border py-2 px-3 mt-2 mb-5 border-gray-300 rounded-md"   />
            </label>
            <label for="parent" class="font-semibold">Parent
              <input v-model="parentName" type="text" required placeholder="Choose a Parent" class="w-full border py-2 px-3 mt-2 mb-5 border-gray-300 rounded-md"   />
            </label>
            <label for="class" class="font-semibold mr-3">Roll
                <input v-model="roll" type="text" required placeholder="Roll" class="w-full border py-2 px-3 mt-2 mb-5 ml-3 border-gray-300 rounded-md" />
            </label>
            <label for="section" class="font-semibold mr-3">Section
              <select v-model="section" class="w-full border py-2 px-3 mt-2 mb-5 border-gray-300 rounded-md" name="class" placeholder="Select Class" id="">
            ...
            <label for="date" class="font-semibold">Admission Date
            ...
            <label for="date" class="font-semibold">Gender
              <div class="w-1/5 flex justify-between">
                <label class="flex">
                <input type="radio" v-model="gender" :value="{ id: 1 }">
                Male
              </label>
              <label>
                <input type="radio" v-model="gender" :value="{ id: 2 }">
                Female
              </label>
              </div>
            </label>
