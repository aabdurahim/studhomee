  ...
  <section class="bg-main h-full">
    ...
    <div class="p-7">
      ...
      <div v-if="selectedClass">
        <select id="class-select" v-model="selectedSection" @change="handleSectionSelect" class="border py-2 px-3 mr-3 border-gray-300 rounded-md">
...
export default {
  ...
  methods: {
    ...
    getSubjects() {
      fetch("http://91.201.214.131:8080/subjects?s=" + this.selectedSection)
        .then(response => response.json())
        .then(data => { 
          this.subjects = data
        })
